    ...
    <div class="max-w-full mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <button wire:click="openImportModal" type="button" class="inline-flex items-center px-4 py-2 bg-gold-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gold-600 transition">
                    <x-heroicon-o-arrow-up-tray class="w-4 h-4 mr-2" />Import
                </button>
                <button wire:click="export" type="button" class="inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 transition">
                    <x-heroicon-o-arrow-down-tray class="w-4 h-4 mr-2" />Export
                </button>
                <button wire:click="openModal" type="button" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 transition">
                    <x-heroicon-o-plus class="w-4 h-4 mr-2" />Tambah Klien
                </button>
            </div>
        </div>
            ...
            <div class="p-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="md:col-span-2">
                        <div class="relative">
                            ...
                            <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-10 pr-3 py-2 border border-gray-300 dark:border-dark-700 rounded-md leading-5 bg-white dark:bg-dark-900 text-gray-900 dark:text-gray-100 placeholder-gray-500 focus:ring-gold-500 focus:border-gold-500 sm:text-sm" placeholder="Cari nama, kode, atau kontak...">
                        </div>
                    </div>
                    <div>
                        <select wire:model.live="statusFilter" class="block w-full px-3 py-2 border border-gray-300 dark:border-dark-700 rounded-md bg-white dark:bg-dark-900 text-gray-900 dark:text-gray-100 focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                ...
                <div class="mt-4 flex items-center">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" wire:model.live="showTrashed" class="rounded border-gray-300 text-gold-600 focus:ring-gold-500">
                        <span class="ml-2 text-sm text-gray-600 dark:text-gray-400"><x-heroicon-o-trash class="w-4 h-4 inline mr-1" />Tampilkan data yang dihapus</span>
                    </label>
                </div>
            </div>
        </div>
                        ...
                        @forelse($clients as $client)
                            <tr class="hover:bg-gray-50 dark:hover:bg-dark-700 {{ $client->trashed() ? 'opacity-60' : '' }}">
                                ...
                                <td class="px-3 py-1.5 text-right text-sm space-x-2">
                                    ...
                                    @else
                                        <button wire:click="openModal({{ $client->id }})" title="Edit" class="text-gold-600 hover:text-gold-900 dark:text-gold-400"><x-heroicon-o-pencil-square class="w-5 h-5" /></button>
                                        <button wire:click="confirmDelete({{ $client->id }}, '{{ $client->name }}')" title="Hapus" class="text-red-600 hover:text-red-900 dark:text-red-400"><x-heroicon-o-trash class="w-5 h-5" /></button>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">@if($showTrashed)Tidak ada klien yang dihapus.@else Belum ada klien. <button wire:click="openModal" class="text-blue-600 hover:underline">Tambah klien</button>@endif</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="p-4 border-t border-gray-200 dark:border-gray-700">{{ $clients->links() }}</div>
        </div>
    </div>
                ...
                <form wire:submit="save">
                    <div class="bg-white dark:bg-dark-800 px-4 pt-5 pb-4 sm:p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="code" value="Kode Klien" />
                                <x-text-input wire:model="code" id="code" type="text" class="mt-1 block w-full" placeholder="Kosongkan untuk otomatis" :disabled="$editingId" />
                                @if(!$editingId)<p class="mt-1 text-xs text-gray-500">Kosongkan untuk kode otomatis (KLN-XXXX)</p>@endif
                            ...
                            <div>
                                ...
                                <x-text-input wire:model="contactPerson" id="contactPerson" type="text" class="mt-1 block w-full" />
                            </div>
                            <div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea wire:model="address" id="address" rows="2" class="mt-1 block w-full border-gray-300 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm"></textarea>
                            </div>
                            <div class="md:col-span-2">
                                <x-input-label for="notes" value="Catatan" />
                                <textarea wire:model="notes" id="notes" rows="2" class="mt-1 block w-full border-gray-300 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-300 focus:border-gold-500 focus:ring-gold-500 rounded-md shadow-sm"></textarea>
                            </div>
                            @if($editingId)
                            <div class="md:col-span-2">
                                <label class="flex items-center"><input type="checkbox" wire:model="isActive" class="rounded border-gray-300 text-gold-600 focus:ring-gold-500"><span class="ml-2 text-sm text-gray-600 dark:text-gray-400">Klien Aktif</span></label>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-dark-800 px-4 pt-5 pb-4 sm:p-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-white">Hapus Klien</h3>
                            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Yakin ingin menghapus <strong>{{ $deleteName }}</strong>?</p>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 dark:bg-dark-700 px-3 py-1.5 sm:px-6 sm:flex sm:flex-row-reverse">
