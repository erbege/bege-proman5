...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <div class="flex items-center space-x-2 text-xs font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2">
            ...
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg>
            <span class="text-slate-500">Bulan {{ $report->month_number }}</span>
        </div>
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white font-outfit tracking-tight">Monthly Report - {{ \Carbon\Carbon::createFromDate($report->year, $report->month, 1)->translatedFormat('F Y') }}</h2>
    </div>
    <div class="flex items-center space-x-3">
        <a href="{{ route('owner.monthly-reports.pdf', $report) }}" class="inline-flex items-center px-4 py-2 bg-slate-100 dark:bg-dark-800 text-slate-700 dark:text-slate-300 text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-slate-200 dark:hover:bg-dark-700 transition border border-slate-200 dark:border-dark-700 shadow-sm">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="portal-card overflow-hidden">
            <div class="relative h-40 bg-slate-900">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-6 text-white">
                    ...
                    <p class="text-xs font-bold text-sky-400 uppercase tracking-widest mt-1">{{ \Carbon\Carbon::parse($report->period_start)->format('d M') }} - {{ \Carbon\Carbon::parse($report->period_end)->format('d M Y') }}</p>
                </div>
            </div>
        ...
        <div class="portal-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-dark-700 bg-slate-50/50 dark:bg-dark-900/50">
                <h3 class="text-sm font-black text-slate-800 dark:text-white uppercase tracking-widest font-outfit">Capaian Pekerjaan (Kumulatif)</h3>
            </div>
                        ...
                        @foreach($rows as $row)
                            ...
                            @else
                                ...
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-dark-700/30 transition group">
                                    ...
                                    <td class="px-4 py-4 text-center text-xs font-bold text-slate-400">{{ number_format($item['weight'], 4) }}%</td>
                                    <td class="px-4 py-4 text-center text-xs font-bold text-sky-600/70">{{ number_format($item['planned']['cumulative'], 4) }}%</td>
                                    <td class="px-4 py-4 text-center text-xs font-bold text-emerald-600">{{ number_format($item['actual']['cumulative'], 4) }}%</td>
                                    <td class="px-4 py-4 text-center text-xs font-bold {{ ($item['actual']['cumulative'] - $item['planned']['cumulative']) >= 0 ? 'text-emerald-500' : 'text-red-500' }}">
                                        {{ number_format($item['actual']['cumulative'] - $item['planned']['cumulative'], 4) }}%
                                    </td>
                                </tr>
                            @endif
                        @endforeach
        ...
        <div class="portal-card p-6">
            <div class="flex items-center justify-between mb-8">
                <h3 class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center">
                    ...
                    Dokumentasi Proyek (Bulan Ini)
                </h3>
            </div>
            
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                @php 
                    $docIds = $report->documentation_ids ?? [];
                    $photos = \App\Models\ProjectFile::whereIn('id', $docIds)->with('latestVersion')->get();
                @endphp
                @forelse($photos as $photo)
                    <div class="group relative aspect-square rounded-lg overflow-hidden bg-slate-100 dark:bg-dark-900 shadow-sm border border-slate-200 dark:border-dark-700">
                        @if($photo->latestVersion)
                            <img src="{{ $photo->latestVersion->getUrl() }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                        @endif
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition duration-300 p-2 flex flex-col justify-end">
                ...
                @empty
                    <div class="col-span-full py-10 text-center border-2 border-dashed border-slate-100 dark:border-dark-700 rounded-lg">
                        <p class="text-slate-400 dark:text-slate-500 text-[10px] italic font-bold">Tidak ada foto dokumentasi bulan ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="portal-card p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-[11px] font-bold text-slate-700 dark:text-slate-300">{{ $report->creator->name ?? 'System' }}</p>
                </div>
                <div>
