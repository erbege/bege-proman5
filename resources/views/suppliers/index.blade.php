<x-app-layout>
    ...
    <div id="supplierModal" class="hidden fixed inset-0 z-50 overflow-y-auto scrollbar-overlay"
        aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" onclick="closeSupplierModal()"
                ...
                <form id="supplierForm" method="POST">
                    ...
                    <div class="bg-white dark:bg-dark-800 px-4 pt-5 pb-4 sm:p-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="modal_code" value="Kode Supplier" />
                                ...
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400" id="codeHint">Kosongkan untuk
                                    kode otomatis (SUP-XXXX)</p>
                            </div>

                            <div>
                                <x-input-label for="modal_name" value="Nama Supplier" />
                            ...
                            <div id="supplierStatusField" class="hidden md:col-span-2">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($suppliers as $supplier)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-3 py-1.5 text-sm text-gray-900 dark:text-white">{{ $supplier->name }}</td>
                                    ...
                                    <td class="px-3 py-1.5 text-right text-sm">
                                        <button type="button" onclick="editSupplier({{ json_encode($supplier) }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                        Belum ada supplier. <button type="button" onclick="openSupplierModal()"
    ...
    @push('scripts')
        <script>
            function openSupplierModal(supplier = null) {
                ...
                const statusField = document.getElementById('supplierStatusField');
                ...
                if (supplier) {
                    ...
                    // Fill form fields
                    document.getElementById('modal_code').value = supplier.code || '';
                    document.getElementById('modal_name').value = supplier.name || '';
                    document.getElementById('modal_contact_person').value = supplier.contact_person || '';
                    document.getElementById('modal_phone').value = supplier.phone || '';
                    document.getElementById('modal_email').value = supplier.email || '';
                    document.getElementById('modal_city').value = supplier.city || '';
                    document.getElementById('modal_address').value = supplier.address || '';
                    document.getElementById('modal_notes').value = supplier.notes || '';
                    document.getElementById('modal_is_active').checked = supplier.is_active;
                } else {
                    // Create mode
