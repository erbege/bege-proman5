<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ $report->cover_title ?? 'Monthly Report ' . \Carbon\Carbon::createFromDate($report->year, $report->month, 1)->translatedFormat('F Y') }}
                </h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $report->period_label }}</p>
            </div>
            @isset($report)
                <div x-data="{}" class="flex items-center space-x-2">
                    @can('monthly_report.manage')
                        <form action="{{ route('projects.monthly-reports.regenerate', [$project, $report]) }}" method="POST"
                            class="inline">
                            @csrf
                            <button type="submit"
                    ...
                    @can('monthly_report.manage')
                        ...
                        @if ($report->can_publish)
                            <button type="button" onclick="document.getElementById('publishModal').classList.remove('hidden')"
                    ...
                    <a href="{{ route('projects.monthly-reports.pdf', [$project, $report]) }}"
                    ...
                    @can('monthly_report.manage')
                        <a href="{{ route('projects.monthly-reports.edit', [$project, $report]) }}"
    ...
    <div class="py-4" x-data="{}">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if ($report->cover_image_url)
                        <div class="aspect-video rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700">
                            <img src="{{ $report->cover_image_url }}" alt="Cover Image"
            ...
            <div class="bg-white dark:bg-dark-800 overflow-hidden shadow-sm sm:rounded-lg p-3" x-data="cumulativeEditor()"
                x-cloak>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        ...
                        <tbody class="bg-white dark:bg-dark-800 divide-y divide-gray-200 dark:divide-gray-700">
                            <template x-for="row in allRows" :key="row.key">
                                <tr
                                    ...
                                    <template x-if="row.type === 'header'">
                                        <td colspan="11"
                                            class="px-2 py-1.5 border text-gray-900 dark:text-white text-xs font-bold"
                            ...
                            <template x-if="!data || !data.sections">
                                <tr>
                                    <td colspan="11"
                                        class="px-3 py-1.5 text-center text-gray-500 dark:text-gray-400">Data
            ...
            <div class="bg-white dark:bg-dark-800 overflow-hidden shadow-sm sm:rounded-lg p-3">
                            ...
                            @if ($doc['url'])
                                <div
                                    ...
                                    <div
                                        ...
                                        <button
                                            @click="window.dispatchEvent(new CustomEvent('open-discussion', { detail: { target: 'photo', id: '{{ $doc['id'] }}', url: '{{ $doc['url'] }}' } }))"
                ...
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-center py-8">Tidak ada dokumentasi yang dipilih.
                    </p>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-dark-800 overflow-hidden shadow-sm sm:rounded-lg p-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <h4 class="font-medium text-gray-900 dark:text-white mb-2">Aktivitas Bulan Ini</h4>
                        <div
                ...
                <div x-show="isOpen"
                    ...
                    <div class="flex h-full flex-col overflow-y-scroll bg-white dark:bg-dark-800 shadow-xl">
                        <div class="px-4 py-6 sm:px-6 bg-indigo-600 text-white">
                            <div class="flex items-start justify-between">
                                ...
                                <div class="ml-3 flex h-7 items-center">
                                    <button @click="isOpen = false" type="button"
                                        class="rounded-md bg-indigo-600 text-indigo-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
                            ...
                            <p class="mt-1 text-sm text-indigo-100">Diskusikan progres dan dokumentasi bulan ini secara
                                real-time.</p>
                        </div>
                        ...
                        <div class="relative flex-1 flex flex-col min-h-0">
                            ...
                            <div x-show="currentTarget" x-transition
                                class="p-4 bg-gray-50 dark:bg-dark-700 border-b dark:border-gray-600">
                                <div class="flex items-center justify-between">
                                    <span
                                        class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Menanggapi
                                        Foto:</span>
                                    <button @click="currentTarget = null"
                                        class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">Hapus
                                        Konteks</button>
                                </div>
                                <div class="mt-2 flex items-center">
                                    <a :href="currentTarget?.url" target="_blank"
                                        class="block hover:opacity-75 transition-opacity">
                                        <img :src="currentTarget?.url"
                                            class="h-16 w-16 object-cover rounded border dark:border-gray-600 shadow-sm">
                                    </a>
                                    <div class="ml-3">
                                        <p class="text-xs text-gray-600 dark:text-gray-300 italic"
                                            x-text="currentTarget?.id"></p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <template x-for="comment in comments" :key="comment.id">
                                    <div class="flex flex-col"
                                        ...
                                        <div x-show="comment.metadata?.target === 'photo'" class="mb-2 w-full">
                                            <a :href="comment.metadata?.url" target="_blank"
                                                ...
                                                <span
                                                    class="mt-1 block text-[8px] text-indigo-600 dark:text-indigo-400 font-bold uppercase">Lihat
                                                    Foto Konteks</span>
                                            </a>
                                        </div>
                            ...
                            <div class="p-4 bg-white dark:bg-dark-800 border-t dark:border-gray-700">
                                <form @submit.prevent="postComment" class="relative">
                                    <textarea x-model="newComment" @keydown.enter.prevent="if(!$event.shiftKey) postComment()" rows="2"
                                    ...
                                    <button type="submit" :disabled="!newComment.trim() || isPosting"
                                        ...
                                        <svg x-show="!isPosting" class="h-6 w-6" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
                                        </svg>
                                        <svg x-show="isPosting" class="h-5 w-5 animate-spin" fill="none"
                                            viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10"
                                                stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor"
        ...
        <script>
            function cumulativeEditor() {
                return {
                    ...
                    canManage: @json(auth()->user()->can('monthly_report.manage')),
                        ...
                        if (window.Echo) {
                            window.Echo.private(`project.${this.projectId}`)
                                .listen('CommentPosted', (e) => {
                                    if (e.commentable_type === 'App\\Models\\MonthlyReport' &&
    ...
    @can('monthly_report.manage')
        <!-- Modals for Workflow -->
        <x-confirm-modal id="submitModal" title="Ajukan Laporan"
            message="Apakah Anda yakin ingin mengajukan laporan ini untuk di-review? Laporan tidak dapat diubah selama proses review."
            confirmColor="blue" icon="document">
            <x-slot name="footer">
                <form action="{{ route('projects.monthly-reports.submit', [$project, $report]) }}" method="POST"
                    class="w-full sm:w-auto">
                    @csrf
                    <button type="submit"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-5 py-2.5 bg-blue-600 text-base font-bold text-white hover:bg-blue-700 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:w-auto sm:text-sm">
                        Ya, Ajukan
                    </button>
                </form>
        ...
        <x-confirm-modal id="approveModal" title="Setujui Laporan"
            message="Apakah Anda yakin ingin menyetujui laporan ini? Laporan yang disetujui dapat dipublish ke Owner."
            confirmColor="green" icon="check">
            <x-slot name="footer">
                <form action="{{ route('projects.monthly-reports.approve', [$project, $report]) }}" method="POST"
                    class="w-full sm:w-auto">
                    @csrf
                    <button type="submit"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-5 py-2.5 bg-emerald-600 text-base font-bold text-white hover:bg-emerald-700 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 sm:w-auto sm:text-sm">
                        Ya, Setujui
                    </button>
                </form>
        ...
        <x-confirm-modal id="rejectModal" title="Tolak Laporan"
            message="Silakan masukkan alasan penolakan. Laporan akan dikembalikan ke pembuat." confirmColor="red"
            icon="exclamation-triangle">
            <x-slot name="footer">
                <form action="{{ route('projects.monthly-reports.reject', [$project, $report]) }}" method="POST"
                    class="w-full">
                    @csrf
                    <div class="mb-4">
                        <textarea name="rejection_reason" rows="3" required placeholder="Alasan penolakan..."
                            class="shadow-sm focus:ring-red-500 focus:border-red-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-dark-700 dark:border-gray-600 dark:text-white"></textarea>
                    </div>
                    <div class="flex justify-end space-x-3">
                        <button type="button" onclick="document.getElementById('rejectModal').classList.add('hidden')"
                            class="inline-flex justify-center rounded-xl border border-gray-200 dark:border-dark-600 shadow-sm px-5 py-2.5 bg-white dark:bg-dark-800 text-base font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all sm:text-sm">
                            Batal
                        </button>
                        <button type="submit"
                            class="inline-flex justify-center rounded-xl border border-transparent shadow-sm px-5 py-2.5 bg-red-600 text-base font-bold text-white hover:bg-red-700 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:text-sm">
                            Tolak Laporan
                        </button>
                    </div>
                </form>
        ...
        <x-confirm-modal id="publishModal" title="Publish Laporan ke Owner"
            ...
            confirmColor="green" icon="check">
            <x-slot name="footer">
                <form action="{{ route('projects.monthly-reports.publish', [$project, $report]) }}" method="POST"
                    class="w-full sm:w-auto">
                    @csrf
                    <button type="submit"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-5 py-2.5 bg-green-600 text-base font-bold text-white hover:bg-green-700 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:w-auto sm:text-sm">
                        Ya, Publish Laporan
                    </button>
                </form>
