<x-app-layout>
    <x-slot name="breadcrumb">
        ...
        ['label' => 'Catat Penggunaan']
    ]" />
    </x-slot>

    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Catat Penggunaan Material - {{ $project->name }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('projects.usage.store', $project) }}" method="POST" x-data="usageForm()">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                <x-input-label for="rab_item_id" value="Pekerjaan Terkait (Opsional)" />
                                ...
                                <x-input-error :messages="$errors->get('rab_item_id')" class="mt-2" />
                            </div>
                                    ...
                                    <thead class="bg-gray-50 dark:bg-dark-700">
                                        <tr>
                                            <th
                                                ...
                                                Material</th>
                                            <th
                                                class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase w-28">
                                                Stok Tersedia</th>
                                            <th
                                            ...
                                            <tr>
                                                <td class="px-4 py-2">
                                                    <x-searchable-select x-bind:name="'items['+index+'][material_id]'"
                                                        x-model="item.material_id" :options="$stockListData"
                                                        options-label="name" options-value="id"
                                                        placeholder="-- Pilih Material --"
                                                        @change="updateInfo(index)" />
                                                </td>
                                                <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">
                                                    <span x-text="item.available + ' ' + item.unit"></span>
                                                </td>
                                                <td class="px-4 py-2">
                                                    <div class="flex items-center">
                                                        <input type="number" :name="'items['+index+'][quantity]'"
                                                            x-model="item.quantity"
                                                            class="block w-24 text-sm border-gray-300 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-300 rounded-md shadow-sm"
                                                            step="0.01" min="0" :max="item.available" required>
                        ...
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('projects.usage.index', $project) }}"
                                class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200">Batal</a>
                            <x-primary-button>
                                Simpan Penggunaan
                            </x-primary-button>
                        </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('usageForm', () => ({
                    stockList: @json($stockListData),
                    items: [{ material_id: '', quantity: '', available: 0, unit: '' }],

                    addItem() {
                        this.items.push({ material_id: '', quantity: '', available: 0, unit: '' });
                    },
                    ...
                    updateInfo(index) {
                        const selectedId = this.items[index].material_id;
                        const stock = this.stockList.find(s => s.id == selectedId);
                        if (stock) {
                            this.items[index].available = stock.quantity;
                            this.items[index].unit = stock.unit;
                        } else {
